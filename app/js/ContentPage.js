...
function alterDOM(request) {
    ...
    if (request.parseDOM) {
		...
		if (document.readyState === "complete") {
			if (enabled) {
				if (angular.element(document.body.getElementsByClassName("DetectedNumber")).length == 0) {
...
function parseDOM(node) {
	...
	if (invalidNodes.indexOf(nodeName) > -1 || angular.element(node).hasClass('dial-message-box')) {
	...
	for (var n = 0; n < resultArray.length; n++) {
		...
		if (cleanNumber.length > telNumber.replace(/[\d\s]/g, '').length && cleanNumber.length >= minTelVal && cleanNumber.length <= maxTelVal) {
			...
			if (insertBefore) {
				...
				angular.element(node).after(spanNode);
...
function buildNode(telNumber, cleanNumber, fullSize) {
	...
	if (fullSize) {
		...
		spanNode.innerHTML = '<div style="display: inline-block; position: relative; cursor: pointer;padding-left:3px;padding-right:20px;-webkit-border-radius: 8px;-webkit-box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;white-space: nowrap;" onclick="clickToCall(' + cleanNumber + ')" title="Call: ' + telNumber + '" class="DetectedNumber">' + telNumber + '<img src="data:image/png;base64,' + handsetImage + '" style="display: inline-block; width: initial; border: 0px; padding: 0px; margin: 0px; float: right; bottom: 1px; right: 1px; position: absolute; background-color:rgba(0,0,0,0);" class="DetectedNumberImg"/></div>';
		// spanNode = $('<div style="display: inline-block; position: relative; cursor: pointer;padding-left:3px;padding-right:20px;-webkit-border-radius: 8px;-webkit-box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;white-space: nowrap;" onclick="location.href=\'' + protocol + '://' + cleanNumber + '\'; return(false)" title="Call: ' + telNumber + '" class="DetectedNumber">' + telNumber + '<img src="data:image/png;base64,' + handsetImage + '" style="display: inline-block; width: initial; border: 0px; padding: 0px; margin: 0px; float: right; bottom: 1px; right: 1px; position: absolute; background-color:rgba(0,0,0,0);" class="DetectedNumberImg"/></div>')[0];
	} else {
		spanNode.innerHTML = '<div style="display: inline-block;cursor: pointer;padding-left:3px;-webkit-border-radius: 8px;-webkit-box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;white-space: nowrap;" onclick="clickToCall(' + cleanNumber + ')" title="Call: ' + telNumber + '" class="DetectedNumber"><img src="data:image/png;base64,' + handsetImage + '" style="width: initial; border: 0px; padding: 0px; margin: 0px; float: none; bottom: 1px; right: 1px; background-color:rgba(0,0,0,0);" class="DetectedNumberImg"/></div>';
		// var spanNode = $('<div style="display: inline-block;cursor: pointer;padding-left:3px;-webkit-border-radius: 8px;-webkit-box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;white-space: nowrap;" onclick="location.href=\'' + protocol + '://' + cleanNumber + '\'; return(false)" title="Call: ' + telNumber + '" class="DetectedNumber"><img src="data:image/png;base64,' + handsetImage + '" style="width: initial; border: 0px; padding: 0px; margin: 0px; float: none; bottom: 1px; right: 1px; background-color:rgba(0,0,0,0);" class="DetectedNumberImg"/></div>')[0];
	}
	return spanNode;
}
...
clickToCall= function (number) {
	debugger;
	var event = new CustomEvent('build', { 'detail': number });
	console.log(number);
    setTimeout(function(){
...
// adding script to allow clicking number to call  
